...
#		var angle_deg: float = lerp(angle_min, angle_max, float(i) / max(n - 1, 1)) + chaos
		...
		if sprite.scale.x < 0:
			angle_deg = 180.0 - angle_deg  # Inverser l'angle si on regarde à gauche
...
func _physics_process(_delta):
	...
	move_and_slide()
		...
		# 🎯 Orientation du sprite selon la direction
	if input_vector.x != 0:
		sprite.scale.x = abs(sprite.scale.x) * sign(input_vector.x)
...
func mettre_a_jour_traceuse():
	...
	# 🎯 Si on regarde à gauche, inverser la direction du vomi !
	if sprite.scale.x < 0:
		direction_2d.x = -direction_2d.x
